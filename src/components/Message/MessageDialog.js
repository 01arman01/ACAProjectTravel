...
import { Avatar, Badge, List, ListItem, ListItemAvatar, ListItemText, TextField } from '@mui/material';
...
export default function MessageDialog({id, url, name, users}) {
  ...
  const handleClickOpen = (scrollType) => () => {
    ...
    messageList.filter(elem=>(elem.sender === id)).filter(el=>el.open===false).forEach((elem)=>{
      if(elem.sender === id && elem.open===false){
        updateDoc(doc(db, "Message",  elem.id), {open:true});
      }
    })
    setMessageList([...messageList.sort(
      (p1, p2) => {
  ...
  React.useEffect(() => {
    ...
    onSnapshot(q, (data) => {
      const newData = data.docs
        ...
        .filter((elem) => ((elem.receiver === userId )&& (elem.sender === id)) || ((elem.receiver === id) && (elem.sender === userId)) );
...
const onSendMessage = async () => {
  ...
  return (
    ...
    <Badge color="secondary" badgeContent={messageList.filter(elem=>(elem.sender === id)).filter(el=>el.open===false).length} showZero>
    <MailIcon onClick={handleClickOpen('paper')} />
      ...
      >
        ...
        <DialogContent dividers={scroll === 'paper'} style={{justifyContent:" flex-end"}}>
          <DialogContentText
            ...
            {messageList.map(
                (elem) =>{
                  if(elem.receiver === id){
                    ...
                    <ListItem button>
                    <ListItemText primary={user.name} secondary={elem.message} />
                      <ListItemAvatar>
        ...
        variant="outlined" onChange={(e)=>setMessage(e.target.value)} />
        
          <Button onClick={handleClose}>Cancel</Button>
          <Button onClick={onSendMessage}><SendIcon /></Button>
        </DialogActions>
      </Dialog>
    </div>
  );
